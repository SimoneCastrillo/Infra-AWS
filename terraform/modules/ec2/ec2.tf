...
upstream backend_servers {
    least_conn;
    server ${aws_instance.backend1.private_ip}:8080;
    server ${aws_instance.backend2.private_ip}:8080;
}
...
resource "aws_instance" "castrillo" {
  ami               = var.ami
  availability_zone = var.a_zone
  instance_type     = var.inst_type
